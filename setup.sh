...
# ── 7b. Apply Soundloader's spotdl patches (shared with Windows setup) ────────
# All patch logic lives in scripts/patch_spotdl.py so macOS and Windows apply
# identical patches. Run with the venv's python so `import spotdl` resolves.
info "Applying spotdl patches..."
SPOTDL_VENV_PYTHON="$PIPX_VENVS/spotdl/bin/python"
if [ ! -f "$SPOTDL_VENV_PYTHON" ]; then
    SPOTDL_VENV_PYTHON="$(dirname "$SPOTDL_PATH")/python"
fi
SCRIPT_DIR_EARLY="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [ -f "$SPOTDL_VENV_PYTHON" ]; then
    "$SPOTDL_VENV_PYTHON" "$SCRIPT_DIR_EARLY/scripts/patch_spotdl.py" \
        || warn "spotdl patching reported an issue — check output above"
    success "spotdl patches applied"
else
